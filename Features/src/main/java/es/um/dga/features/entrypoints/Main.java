...
//                r -> new Thread(r, "Malware Families Pool"));
        ...
        */
        ...
        for (String malwareVariant : malwareFamilies) {
            String malwareVariantDirectory = Settings.getDataFolder() + malwareVariant;
    ...
    class MalwareFamily implements Callable<MalwareFamily> {
        ...
        public MalwareFamily call() throws Exception {
            ...
            //File file = new File(this.listDirectory.getAbsolutePath() + "/10000.txt");
            {
                ...
                InternetDomainName firstFQDN = internetDomainNames.stream().findFirst().map(domainName -> {
                    internetDomainNames.remove(domainName);
                    return domainName;
                ...
                for (InternetDomainName internetDomainName : internetDomainNames) {
                    futureLines.add(makeCompletableFuture(localExecutor.submit(() -> {
                        ...
                        analyzer = null;
                        String formattedFeatureValue = arffHelper.getFormattedFeatureValue(allFeatures);
                        allFeatures = null;
                        // avoid double memory
...
*/
                ...
                File arffFile = new File(
                        this.root.getAbsolutePath() + "/arff/" + file.getName() + Settings.getFeatureCapabilities()
                                + ".arff");
                ...
                File csvFile = new File(
                        this.root.getAbsolutePath() + "/csv/" + file.getName() + Settings.getFeatureCapabilities()
                                + ".csv");
                ...
                */
                ...
                Settings.getLogger().log(Level.INFO,
                        String.format("%1$16s", file.getName()) + "\t" + humanReadableDifference(startL, startan) + "\t"
                                + humanReadableDifference(startan, startWA) + "\t" + humanReadableDifference(startWA,
                                startWC) + "\t" + humanReadableDifference(startWC, LocalDateTime.now()) + "\t"
                                + humanReadableDifference(start, LocalDateTime.now()));
